...
/**
 * Helper component to encapsulate Tooltip logic for buttons
 */
const ControlButton = ({
  children,
  tooltip,
    ...
    <TooltipProvider delayDuration={300}>
      <div className="absolute bottom-24 lg:bottom-6 left-1/2 -translate-x-1/2 lg:left-auto lg:right-6 lg:translate-x-0 flex items-center gap-3 z-50 p-2 rounded-2xl glass border-surface-800/30 pointer-events-auto">
        <ControlButton
          ...
          tooltip={isAutoRotating ? 'Stop rotation' : 'Start rotation'}
          ariaLabel={isAutoRotating ? 'Stop rotation' : 'Start rotation'}
          ariaPressed={isAutoRotating}
        >
        ...
        <ControlButton
          ...
          tooltip={isFocusMode ? 'Show UI' : 'Hide UI (Focus Mode)'}
          ariaLabel={isFocusMode ? 'Show UI' : 'Hide UI (Focus Mode)'}
          ariaPressed={isFocusMode}
        >
          {isFocusMode ? (
             <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
